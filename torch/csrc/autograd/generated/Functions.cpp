...
variable_list GridSampler2DBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ input_ix, grid_ix })) {
    ...
    auto grad_result = grid_sampler_2d_backward(grad, input, grid, interpolation_mode, padding_mode);
...
variable_list GridSampler3DBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ input_ix, grid_ix })) {
    ...
    auto grad_result = grid_sampler_3d_backward(grad, input, grid, interpolation_mode, padding_mode);
      if (should_compute_output({ input_ix })) {
        copy_range(grad_inputs, input_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ grid_ix })) {
...
variable_list IndexBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix })) {
    auto grad_result = index_backward(self, indices, grad);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list NormBackward2::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.scalar_type()), p, result).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list NormBackward3::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.scalar_type()), p, result, dim, keepdim).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CdistBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ x1_ix })) {
    auto grad_result = _cdist_backward(grad, x1, x2, p, result);
...
variable_list ProdBackward0::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.scalar_type()), result).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list ProdBackward1::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = prod_backward(grad, self.to(grad.scalar_type()), result, dim, keepdim).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list PstrfBackward::apply(variable_list&& grads) {
...
variable_list SumBackward0::apply(variable_list&& grads) {
  ...
  auto& grad = grads[0];
  if (should_compute_output({ self_ix })) {
    auto grad_result = grad.expand(self_sizes).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list SumBackward1::apply(variable_list&& grads) {
  ...
  auto& grad = grads[0];
  if (should_compute_output({ self_ix })) {
    auto grad_result = sum_backward(grad, self_sizes, dim, keepdim).to(self_scalar_type);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ConvTranspose2DBackward::apply(variable_list&& grads) {
...
variable_list ConvTranspose3DBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix, weight_ix, bias_ix })) {
    ...
    auto grad_result = conv_transpose3d_backward(grad, self, weight, kernel_size, stride, padding, output_padding, dilation, empty_like(grad), empty_like(grad), grad_input_mask);
      if (should_compute_output({ self_ix })) {
        copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ weight_ix })) {
...
variable_list ConvTranspose3DBackwardBackward::apply(variable_list&& grads) {
...
variable_list ConvDilated2DBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix, weight_ix, bias_ix })) {
    ...
    auto grad_result = conv_dilated2d_backward(grad, self, weight, kernel_size, stride, padding, dilation, grad_input_mask);
      if (should_compute_output({ self_ix })) {
        copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ weight_ix })) {
...
variable_list ConvDilated2DBackwardBackward::apply(variable_list&& grads) {
...
variable_list ConvDilated3DBackward::apply(variable_list&& grads) {
  ...
  if (should_compute_output({ self_ix, weight_ix, bias_ix })) {
    ...
    auto grad_result = conv_dilated3d_backward(grad, self, weight, kernel_size, stride, padding, dilation, grad_input_mask);
      if (should_compute_output({ self_ix })) {
        copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
      }
      if (should_compute_output({ weight_ix })) {
...
variable_list ConvDilated3DBackwardBackward::apply(variable_list&& grads) {
